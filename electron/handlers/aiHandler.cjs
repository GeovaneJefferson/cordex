...
module.exports = function(mainWindow) {
  ...
  ipcMain.handle('ai:fix-error', async (_ev, { errorMessage, filePath, line, column, codeSnippet }) => {
    ...
    try {
      const res = await llamaGenerate({ model, systemPrompt: 'You are an expert developer. Return only valid JSON.', prompt, stream: false, temperature: 0.1, num_predict: 1024 })
      const response = (await extractText(res)).trim()
